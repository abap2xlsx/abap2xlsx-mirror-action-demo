...
* Theme not supported yet - start with RGB
  ...
  METHOD add_sheet_views.
    ...
    IF lv_freeze_cell_row IS NOT INITIAL AND lv_freeze_cell_column IS NOT INITIAL.
      ...
      lv_freeze_cell_column_alpha = ycl_ecb_common=>convert_column2alpha( ip_column = lv_freeze_cell_column ).
      lv_value = ycl_ecb_common=>number_to_excel_string( ip_value = lv_freeze_cell_row ).
